...
# -*- coding: utf-8 -*-
"""
formal_analysis_v3.py  -  Study 2 正式分析脚本（基于 v2，按 formal_report_v2-final.md 结构调整）

改动说明（vs v2）:
  1. 学历使用五分法（edu_ord 1-5），所有涉及人口统计学的分析均重算
     参照组 = 3 本科；新增哑变量：edu_hs / edu_sc / edu_ma / edu_phd
  2. 补充大回归（3.4）：同时控制人口统计学 + AI素养变量
  3. 调节效应斜率图升级为 PROCESS/SPSS 风格（含置信带、-1SD/M/+1SD 标注）
  4. 补充策略使用程度分析（九）：usage_degree + 词频分析 + 聚类分析
  5. 删除以下模块：五（信心校准）、八（异质性）、十（相关矩阵）、
     十一（综合结论）、十二（干预专项）、旧3.4（层次回归）、6.4（AI来源）、7.5（AIE简单斜率）

输出:
  analysis/output_1/formal_report_v3.md
  analysis/output_1/F4_moderation_v3.png  (PROCESS 风格)
  analysis/output_1/F7_strategy_wordfreq.png
  analysis/output_1/F8_strategy_cluster.png
"""
...
log_path = os.path.join(OUT_DIR, 'formal_report_v3.md')
...
pr('> 模型三：同一模型中同时纳入人口统计学（性别+年龄+学历五分法）和AI素养变量（AI频率+前测自评），\n'
