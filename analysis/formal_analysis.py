...
log_path = os.path.join(OUT_DIR, 'formal_report.md')
...
md_table(
    ...
    [
        ...
        ['Manipulation Check（实验组）',f'实验组未通过 MC 者排除 {n_mc_fail} 人', f'**{n_final}**（对照={nA}, 实验={nC})'],
    ]
)
...
cont_rows = []
for col, label in [('ai_familiarity',      'AI 熟悉度（1–5）'),
                   ('self_assessed_ability','前测自评辨别能力（1–5）'),
...
_DEMO_FORMULA = ('gender_female + '
                 ...
                 'edu_lower + edu_higher')
_AI_FORMULA   = 'ai_familiarity + self_assessed_ability + ai_exposure_num'

_DEMO_COLS = ['gender_female', 'age_25_34', 'age_35_44', 'age_45_54', 'edu_lower', 'edu_higher']
_AI_COLS   = ['ai_familiarity', 'self_assessed_ability', 'ai_exposure_num']
...
t3a.to_csv(os.path.join(OUT_DIR, 'table3a_reg_accuracy_demo.csv'), index=False, encoding='utf-8-sig')
...
t4a.to_csv(os.path.join(OUT_DIR, 'table4a_reg_dprime_demo.csv'), index=False, encoding='utf-8-sig')

h3('3.3 回归分析（控制AI素养相关）：DV = 整体正确率 & d\'')
pr('> 控制变量：AI熟悉度（1–5）、前测自评能力（1–5）、AI使用频率（1–5）；均为连续变量。\n')
...
t3b.to_csv(os.path.join(OUT_DIR, 'table3b_reg_accuracy_ai.csv'), index=False, encoding='utf-8-sig')
...
t4b.to_csv(os.path.join(OUT_DIR, 'table4b_reg_dprime_ai.csv'), index=False, encoding='utf-8-sig')
...
md_table(
    ...
    [
        ...
        ['M3', '+前测自评能力',
         fmt(m3.params["group_c"],3),
         f'[{m3.conf_int().loc["group_c",0]:.3f}, {m3.conf_int().loc["group_c",1]:.3f}]',
         fmt_r(m3.rsquared), f'{m3.rsquared-m2.rsquared:+.3f}',
         fmt_p(m3.pvalues["group_c"]) + stars(m3.pvalues["group_c"])],
    ]
)
pr(f'\n> **注**: 组别效应在 M1 中显著（粗效应），加入前测自评能力后减弱/消失（M3），'
   f'表明两组基线能力差异（尽管不显著）在一定程度上解释了准确率差异。结论应聚焦控制协变量后的回归结果。')
...
t5a.to_csv(os.path.join(OUT_DIR, 'table5a_reg_calibration_demo.csv'), index=False, encoding='utf-8-sig')
...
t5b.to_csv(os.path.join(OUT_DIR, 'table5b_reg_calibration_ai.csv'), index=False, encoding='utf-8-sig')
...
pr('> **注**: 模型使用 self_assessed_ability 的中心化版本 sae_c，同时包含其余控制变量（不重复纳入 self_assessed_ability 原始值）。\n')

mod_df   = master.dropna(subset=['acc_total','self_assessed_ability','group_c']+CTRL_VARS_MOD).copy()
mod_df['sae_c']  = mod_df['self_assessed_ability'] - mod_df['self_assessed_ability'].mean()
sd_sae   = mod_df['self_assessed_ability'].std(ddof=1)
mean_sae = mod_df['self_assessed_ability'].mean()
...
ss_rows = []
for level, label in [(-sd_sae, f'低自评 −1SD (SAE≈{mean_sae-sd_sae:.2f})'),
                     (0.0,     f'均值     (SAE≈{mean_sae:.2f})'),
                     (+sd_sae, f'高自评 +1SD (SAE≈{mean_sae+sd_sae:.2f})')]:
    mod_df['sae_tmp'] = mod_df['sae_c'] - level
    m_tmp = smf.ols(f'acc_total ~ group_c * sae_tmp + {ctrl_str_mod}', data=mod_df).fit()
    ...
    ss_rows.append([label, fmt(b,3), fmt(se,3),
                    f'[{ci_lo:.3f}, {ci_hi:.3f}]',
                    fmt(t_,3), fmt_p(p_)+stars(p_)])
md_table(['水平', 'B（组别效应）', 'SE', '95% CI', 't', '*p*'], ss_rows)

int_coef = m_mod.params.get('group_c:sae_c', np.nan)
b_grp    = m_mod.params.get('group_c', np.nan)
se_grp   = m_mod.bse.get('group_c', np.nan)
...
pr('> **注**: 模型使用 ai_exposure_num 的中心化版本 aie_c；控制变量包含 self_assessed_ability，'
   '但不重复纳入 ai_exposure_num 原始值。\n')
...
aie_base = master.dropna(subset=['acc_total','ai_exposure_num','group_c']+_CTRL_AIE).copy()
aie_base['aie_c'] = aie_base['ai_exposure_num'] - aie_base['ai_exposure_num'].mean()
sd_aie   = aie_base['ai_exposure_num'].std(ddof=1)
mean_aie = aie_base['ai_exposure_num'].mean()

m_aie = smf.ols(f'acc_total ~ group_c * aie_c + {_ctrl_aie_str}', data=aie_base).fit()
X_aie_raw = aie_base[['group_c','aie_c'] + _CTRL_AIE]
...
aie_ss_rows = []
for level, label in [(-sd_aie, f'低频率 −1SD (AIE≈{mean_aie-sd_aie:.2f})'),
                     (0.0,     f'均值     (AIE≈{mean_aie:.2f})'),
                     (+sd_aie, f'高频率 +1SD (AIE≈{mean_aie+sd_aie:.2f})')]:
    aie_base['aie_tmp'] = aie_base['aie_c'] - level
    m_tmp = smf.ols(f'acc_total ~ group_c * aie_tmp + {_ctrl_aie_str}', data=aie_base).fit()
    b = m_tmp.params['group_c']; se = m_tmp.bse['group_c']
    t_ = m_tmp.tvalues['group_c']; p_ = m_tmp.pvalues['group_c']
    ci_lo = m_tmp.conf_int().loc['group_c',0]; ci_hi = m_tmp.conf_int().loc['group_c',1]
    aie_ss_rows.append([label, fmt(b,3), fmt(se,3),
                        f'[{ci_lo:.3f}, {ci_hi:.3f}]',
                        fmt(t_,3), fmt_p(p_)+stars(p_)])
md_table(['水平', 'B（组别效应）', 'SE', '95% CI', 't', '*p*'], aie_ss_rows)

int_coef_aie = m_aie.params.get('group_c:aie_c', np.nan)
b_grp_aie    = m_aie.params.get('group_c', np.nan)
se_grp_aie   = m_aie.bse.get('group_c', np.nan)
if not np.isnan(int_coef_aie) and abs(int_coef_aie) > 1e-10:
    try:
        jn1 = (-b_grp_aie - 1.96*se_grp_aie) / int_coef_aie
        jn2 = (-b_grp_aie + 1.96*se_grp_aie) / int_coef_aie
        ...
        pr(f'→ group 效应在此区间**外**达 _p_ < .05（交互方向 {">" if int_coef_aie>0 else "<"} 0）')
    except Exception:
        pr('> JN 计算失败')
...
for dv, tbl_label, tbl_fname in [
    ('acc_total',       '辨别能力异质性（T7）',  'table7_heterogeneity_accuracy.csv'),
    ('calibration_gap', '信心校准异质性（T8）',  'table8_heterogeneity_calibration.csv'),
]:
    h3(f'8.{"1" if "acc" in dv else "2"} {tbl_label}')
    het_rows = []
    for k1, k2, pair_label in pair_keys:
...
corr_vars = ['acc_total','dprime','self_assessed_ability','ai_familiarity',
             'ai_exposure_num','calibration_gap','efficacy_change_proxy']
corr_labels = ["准确率","d'","前测自评","AI熟悉度","AI使用频率","校准差距","效能变化代理"]
...
try:
    ...
    plt.tight_layout()
    fig_path = os.path.join(OUT_DIR, 'F3_correlation_heatmap.png')
...
for col, lbl in [('ai_familiarity','AI熟悉度'),
                  ('self_assessed_ability','前测自评能力'),
