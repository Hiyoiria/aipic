...
| 变量          | B     | SE    | Beta  | t     | p      | VIF   |
| ----------- | ----- | ----- | ----- | ----- | ------ | ----- |
| (常量)        | 0.419 | 0.057 |       | 7.331 | < .001 |       |
| 是否进行信息干预    | 0.064 | 0.029 | 0.205 | 2.222 | .029*  | 1.000 |
| AI使用频率（1–5） | 0.023 | 0.013 | 0.166 | 1.739 | .085   | 1.076 |
| 前测自评能力（1–5） | 0.030 | 0.013 | 0.217 | 2.270 | .025*  | 1.076 |

_R²=0.134, Adj.R²=0.109, F(3,102)=5.277, p = .002**_
_因变量：识别准确率（模型二：控制AI素养相关）_

> **残差诊断**: Shapiro-Wilk *W* = 0.983, *p* = .204 （正态）; Breusch-Pagan *p* = .893 （同方差）; Durbin-Watson = 0.28
...
| 变量          | B      | SE    | Beta  | t      | p     | VIF   |
| ----------- | ------ | ----- | ----- | ------ | ----- | ----- |
| (常量)        | -0.523 | 0.312 |       | -1.676 | .097  |       |
| 是否进行信息干预    | 0.342  | 0.158 | 0.199 | 2.167  | .033* | 1.000 |
| AI使用频率（1–5） | 0.135  | 0.073 | 0.176 | 1.852  | .067  | 1.076 |
| 前测自评能力（1–5） | 0.174  | 0.073 | 0.226 | 2.374  | .019* | 1.076 |

_R²=0.141, Adj.R²=0.116, F(3,102)=5.600, p = .001**_
_因变量：敏感性指标 d'（模型二：控制AI素养相关）_

> **残差诊断**: Shapiro-Wilk *W* = 0.990, *p* = .652 （正态）; Breusch-Pagan *p* = .520 （同方差）; Durbin-Watson = 0.31
...
| M2  | +前测自评能力 | 0.064 | [0.007, 0.122] | .109 | +0.068 | .030*   |

> **注**: 组别效应在 M1 中显著（粗效应），加入前测自评能力后减弱/消失（M2），表明两组基线自评能力差异（尽管不显著）在一定程度上解释了准确率差异。结论应聚焦控制协变量后的回归结果。
...
| 变量          | B      | SE    | Beta   | t      | p    | VIF   |
| ----------- | ------ | ----- | ------ | ------ | ---- | ----- |
| (常量)        | 0.086  | 0.077 |        | 1.121  | .265 |       |
| 是否进行信息干预    | -0.009 | 0.039 | -0.023 | -0.231 | .818 | 1.000 |
| AI使用频率（1–5） | -0.020 | 0.018 | -0.114 | -1.122 | .265 | 1.076 |
| 前测自评能力（1–5） | -0.001 | 0.018 | -0.003 | -0.033 | .974 | 1.076 |

_R²=0.014, Adj.R²=-0.015, F(3,102)=0.476, p = .700_
_因变量：信心校准差距（模型二：控制AI素养相关）_

> **残差诊断**: Shapiro-Wilk *W* = 0.982, *p* = .173 （正态）; Breusch-Pagan *p* = .521 （同方差）; Durbin-Watson = 1.20
...
| 变量           | B      | SE    | 95% CI           | β    | t      | p         | VIF  |
| ------------ | ------ | ----- | ---------------- | ---- | ------ | --------- | ---- |
| 截距           | 0.592  | 0.020 | [0.553, 0.632]   | —    | 29.645 | —         | —    |
| 组别（C=1）      | 0.064  | 0.029 | [0.008, 0.121]   | .205 | 2.252  | .026*     | 1.00 |
| 前测自评能力（中心化）  | 0.065  | 0.018 | [0.030, 0.100]   | .462 | 3.693  | < .001*** | 1.00 |
| 交互：组别 × 自评能力 | -0.060 | 0.026 | [-0.111, -0.009] | —    | -2.342 | .021*     | —    |

_R² = .154, Adj.R² = .129, F(3, 102) = 6.196, p < .001***_

> **残差诊断**: Shapiro-Wilk *W* = 0.986, *p* = .326 （正态）; Breusch-Pagan *p* = .797 （同方差）; Durbin-Watson = 0.32
...
| 水平                  | B（组别效应） | SE    | 95% CI          | t      | *p*    |
| ------------------- | ------- | ----- | --------------- | ------ | ------ |
| 低自评 −1SD (SAE≈1.94) | 0.131   | 0.040 | [0.051, 0.212]  | 3.250  | .002** |
| 均值     (SAE≈3.06)   | 0.064   | 0.029 | [0.008, 0.121]  | 2.252  | .026*  |
| 高自评 +1SD (SAE≈4.18) | -0.003  | 0.041 | [-0.083, 0.077] | -0.074 | .941   |

Johnson-Neyman 近似显著性边界（中心化 sae_c）: 0.139 到 2.001
→ 对应原始 self_assessed_ability: 3.20 到 5.06
→ group 效应在此区间**外**达 *p* < .05（交互方向 < 0）
...
| 变量           | B     | SE    | 95% CI          | β    | t      | p     | VIF  |
| ------------ | ----- | ----- | --------------- | ---- | ------ | ----- | ---- |
| 截距           | 0.593 | 0.021 | [0.553, 0.634]  | —    | 28.855 | —     | —    |
| 组别（C=1）      | 0.063 | 0.029 | [0.005, 0.121]  | .201 | 2.147  | .034* | 1.00 |
| AI使用频率（中心化）  | 0.017 | 0.018 | [-0.019, 0.052] | .119 | 0.930  | .355  | 1.00 |
| 交互：组别 × AI频率 | 0.032 | 0.026 | [-0.020, 0.084] | —    | 1.216  | .227  | —    |

_R² = .104, Adj.R² = .077, F(3, 102) = 3.930, p = .011*_

> **残差诊断**: Shapiro-Wilk *W* = 0.990, *p* = .600 （正态）; Breusch-Pagan *p* = .035* （疑似异方差）; Durbin-Watson = 0.23
...
| 水平                  | B（组别效应） | SE    | 95% CI          | t     | *p*   |
| ------------------- | ------- | ----- | --------------- | ----- | ----- |
| 低频率 −1SD (AIE≈2.36) | 0.027   | 0.042 | [-0.056, 0.110] | 0.649 | .518  |
| 均值     (AIE≈3.48)   | 0.063   | 0.029 | [0.005, 0.121]  | 2.147 | .034* |
| 高频率 +1SD (AIE≈4.60) | 0.099   | 0.042 | [0.016, 0.182]  | 2.375 | .019* |

Johnson-Neyman 近似显著性边界（中心化 aie_c）: -3.765 到 -0.171
→ 对应原始 ai_exposure_num: -0.28 到 3.31
→ group 效应在此区间**外**达 _p_ < .05（交互方向 > 0）
...
| AI使用频率 | 0.224*    | 0.236*    | 0.266**   | 1.000  | —        | —      |
| 校准差距   | -0.554*** | -0.524*** | -0.033    | -0.115 | 1.000    | —      |
| 效能变化代理 | -0.039    | -0.034    | -0.749*** | -0.186 | 0.465*** | 1.000  |
